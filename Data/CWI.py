...
#print(len(words))
#matrix = np.empty([100])
word_stats = pd.DataFrame(columns=['word', 'count'])
...
for index, row in dataset.iterrows():
        ...
        for g_index, g_row in df_by_group.iterrows():
            ...
            if len(rating_subset) == 0:
                ...
                print(rating)
            if not word_stats['word'].isin([g_row['word']]).any():
                word_stats.loc[len(word_stats)] = [g_row['word'],1]
            else:
                word_stats.loc[word_stats['word']==g_row['word'],['count']] = word_stats.loc[word_stats['word']==g_row['word'],['count']] + 1
print(rating)
rating.to_csv('groupwise_data.csv')
print(word_stats)
word_stats.to_csv('wordstats.csv')
