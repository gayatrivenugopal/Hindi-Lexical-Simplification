...
#property
def get_number_of_synonyms(word):
    number = 0
    synsets = get_synsets(word)
    for synset in synsets:
        number += get_number_of_synonyms_in_synset(synset)
    return number
    
#property
def get_average_number_of_synonyms(word):
    number = 0
    synsets = get_synsets(word)
    for synset in synsets:
        number += get_number_of_synonyms_in_synset(synset)
    return number/len(synsets)
...
def get_properties(word):
    """ Create a dictionary of all the properties.
    
    Argument: word (str): the word whose properties are to be retrieved
    Returns: (dict): keys are the property names and values are the 
    corresponding property values
    get_length
    get_number_of_synsets
    get_number_of_synonyms
    get_average_number_of_synonyms
    is_person
    is_place
    get_number_of_consonants
    get_number_of_vowels
    get_number_of_consonant_conjuncts
    get_syllable_count
    """
    properties = dict()
    properties['length'] = get_length(word)
    properties['n_synsets'] = get_number_of_synsets(word)
    #properties['n_synsets'] = get_sense_count(word)
    properties['n_synonyms'] = get_number_of_synonyms(word)
    properties['n_avg_synonyms'] = get_average_number_of_synonyms(word)
    properties['n_consonants'] = get_number_of_consonants(word)
    properties['n_vowels'] = get_number_of_vowels(word)
    properties['n_consonantconjuncts'] = get_number_of_consonant_conjuncts(word)
    properties['n_syllables'] = get_syllable_count(word)
    return properties
    #TODO: frequency
